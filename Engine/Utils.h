...
namespace Bcg {
    ...
    std::vector<Real> ParseNumbers(const std::string &line) {
    ...
    void ParseNumbers(const std::string &str,
        ...
        auto mystrtol = [](const char*& pen) -> uint64_t {
            uint64_t val = 0;
        ...
        while (*pen != '\0') {
            ...
            if (std::isdigit(*pen)) {
                ...
                if ((*pen | ('E' ^ 'e')) == 'e') {
                    switch (*++pen) {
                        case '+':
                            neg_exp -= static_cast<std::ptrdiff_t>(mystrtol(++pen));
                            break;
                        case '-':
                            neg_exp += static_cast<std::ptrdiff_t>(mystrtol(++pen));
                            break;
