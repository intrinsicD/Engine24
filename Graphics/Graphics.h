...
namespace Bcg {
    ...
    public:
        PluginGraphics() : Plugin("Graphics") {}

        ~PluginGraphics() override = default;

        static void *create_window(int width, int height, const char *title);

        static void destroy_window(void *window);

        static void register_callbacks(void *window);
