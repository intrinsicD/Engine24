			...
			for (uint s_ = 0; s_ < nParents; ++s_)
			{
				...
				for (uint i_ = 0; i_ < resultSet.size(); ++i_)
				{
					...
					if (kld > params.alpha * params.alpha * 0.5f)
						//if ( squaredMahalanobisDist( child.Âµ - parent.Âµ, parent.cov ) > params.alpha*params.alpha )
						//if (!child.delta && squaredMahalanobisDist( child.Âµ - parent.Âµ, parent.cov ) > params.alpha * params.alpha)
						continue;
