...
namespace Bcg {
    ...
    public:
        ...
        void query(const AABB<float> &query_aabb, std::vector<size_t> &result) const {
            ...
            auto vol_aabb = [](const AABB<float> &b) {
            ...
            while (!stack.empty()) {
                ...
                if (!IntersectsTraits<AABB<float>, AABB<float> >::intersects(node.aabb, query_aabb)) continue;
                ...
                if (strictly_larger &&
                    ContainsTraits<AABB<float>, AABB<float> >::contains(query_aabb, node.aabb)) {
                ...
                if (node.is_leaf) {
                    for (size_t i = 0; i < node.num_elements; ++i) {
                        ...
                        if (IntersectsTraits<AABB<float>, AABB<float> >::intersects(element_aabbs[ei], query_aabb)) {
                            result.push_back(ei);
                        }
                    }
                } else {
                    ...
                    for (size_t ci: node.children) {
                        if (ci != std::numeric_limits<size_t>::max() &&
                            IntersectsTraits<AABB<float>, AABB<float> >::intersects(nodes[ci].aabb, query_aabb)) {
                            stack.push_back(ci);
                        }
                    }
                }
        ...
        void query(const Sphere<float> &query_sphere, std::vector<size_t> &result) const {
            ...
            while (!stack.empty()) {
                ...
                if (!IntersectsTraits<AABB<float>, Sphere<float> >::intersects(node.aabb, query_sphere)) continue;

                const bool strictly_larger = (vol_sphere(query_sphere) > vol_aabb(node.aabb) + eps);

                if (strictly_larger &&
                    ContainsTraits<Sphere<float>, AABB<float> >::contains(query_sphere, node.aabb)) {
                    for (size_t i = 0; i < node.num_elements; ++i) {
                        result.push_back(element_indices[node.first_element + i]);
                    }
                    continue;
                ...
                if (node.is_leaf) {
                    ...
                    for (size_t ci: node.children) {
                        if (ci != std::numeric_limits<size_t>::max() &&
                            IntersectsTraits<AABB<float>, Sphere<float> >::intersects(nodes[ci].aabb, query_sphere)) {
                            stack.push_back(ci);
                        }
                    }
                }
        ...
        void query_knn(const Vector<float, 3> &query_point, size_t k, std::vector<size_t> &results) const {
            ...
            auto d2_node = [&](size_t ni) {
                return SquaredDistanceTraits<AABB<float>, Vector<float, 3> >::squared_distance(
            ...
            while (!pq.empty()) {
                ...
                if (node.is_leaf) {
                    ...
                    for (size_t i = 0; i < node.num_elements; ++i) {
                        ...
                        const float elem_dist_sq = SquaredDistanceTraits<AABB<float>, Vector<float,
                            3> >::squared_distance(
    ...
    private:
        ...
        void subdivide_volume(const size_t node_idx, size_t depth) {
            ...
            std::array<AABB<float>, 8> octant_aabbs;
            for (int j = 0; j < 8; ++j) {
                ...
                octant_aabbs[j] = AABB<float>(child_min, child_max);
            }
            ...
            for (size_t i = 0; i < node.num_elements; ++i) {
                ...
                if (elem_aabb.min == elem_aabb.max) {
                    ...
                    for (int j = 0; j < 8; ++j) {
                        if (ContainsTraits<AABB<float>, AABB<float> >::contains(octant_aabbs[j], elem_aabb)) {
                            if (found_child == -1) {
                                found_child = j;
                            } else {
